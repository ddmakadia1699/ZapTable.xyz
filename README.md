...
4. **Pay** from the phone (UPI / card / Apple Pay / Google Pay).
...
- 💳 **Payments** — UPI / Razorpay (India) and Stripe (cards, Apple/Google Pay) for global.
...
- **Payments:** Stripe + Razorpay/UPI — multi-gateway, multi-currency. **Funds settle
  directly into the restaurant's account**; ZapTable is a flat-fee SaaS, not a money
  processor (keeps the legal/accounting overhead off us).
- **QR codes:** generated per table, exported as a printable PDF.

> 🏗️ The multi-tenant SaaS architecture (Supabase + AWS), data model, settings, and the
...
A simple, predictable **flat monthly subscription** — no per-order cut. (Owners hate
revenue-based fees; a flat utility bill is easier to say yes to.)

| Tier | For | Price |
| --- | --- | --- |
| **Lite** | Small cafes, food trucks, stalls | ~$49 / ₹999 per month |
| **Pro** | Casual restaurants, busy bars | ~$129 / ₹2,999 per month |
| **Enterprise** | Multi-location chains, food courts | Custom |

_Prices indicative; supports INR and USD from day one._
...
- ⏱️ **Live queue** — "X orders ahead, ~Y min," with kitchen-load-aware throttling.
- 💬 **Conversational AI ordering** — "veg, spicy, under ₹300" → recommendations from the menu.
